...
const API_URL = "http://localhost:3001/backlog"; // URL de la API para obtener los backlogs
...
> => {
  try {
    ...
    const response = await axios.get<{ tasks: ITarea[] }>(API_URL);
    return response.data.tasks; // Retornamos la lista de tasks
  } catch (error) {
